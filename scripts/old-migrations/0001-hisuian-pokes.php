<?php

declare(strict_types=1);
die('This file is not meant to be executed anymore, its just kept here for reference.');
require_once __DIR__ . '/_bootstrap.php';

// Edit and run this script to quickly modify/migrate the structure of the pokemon.json data file.
//          Usage:    php scripts/_migrate.php
//
// Do not commit migration-specific changes of this file to the repository, unless they are commented out.
//

(static function () {
    error_reporting(-1);

    // template to add new fields in particular positions (or sort existing), to the pokemon entries
    $dataTemplate = <<<JSON
{
    "id": null,
    "nid": null,
    "dexNum": null,
    "formId": null,
    "name": null,
    "formName": null,
    "region": null,
    "generation": -1,
    "type1": null,
    "type2": null,
    "color": null,
    "abilities": {
        "primary": null,
        "secondary": null,
        "hidden": null
    },
    "isLegendary": false,
    "isMythical": false,
    "isDefault": false,
    "isForm": false,
    "isSpecialAbilityForm": false,
    "isCosmeticForm": false,
    "isFemale": false,
    "hasGenderDifferences": false,
    "isBattleOnlyForm": false,
    "isSwitchableForm": false,
    "isFusion": false,
    "fusedWith": null,
    "isMega": false,
    "isPrimal": false,
    "isRegional": false,
    "isGmax": false,
    "canGmax": false,
    "canDynamax": false,
    "canBeAlpha": false,
    "obtainableIn": [],
    "storableIn": ["home"],
    "shinyReleased": false,
    "baseStats": {
        "hp": -1,
        "atk": -1,
        "def": -1,
        "spa": -1,
        "spd": -1,
        "spe": -1
    },
    "goStats": {
        "maxCP": -1,
        "attack": -1,
        "defense": -1,
        "stamina": -1
    },
    "weight": {
        "avg": "-1kg",
        "min": "-1kg",
        "max": "-1kg",
        "alpha": "-1kg"
    },
    "height": {
        "avg": "-1m",
        "min": "-1m",
        "max": "-1m",
        "alpha": "-1m"
    },
    "maleRate": -1,
    "femaleRate": -1,
    "refs": {
        "pogo": null,
        "veekunDb": null,
        "showdownMarkup": null,
        "showdownData": null,
        "serebii": null,
        "bulbapedia": null
    },
    "baseSpecies": null,
    "baseForms": [],
    "forms": [],
    "evolutions": []
}
JSON;

    $dataTemplateArr = sgg_json_decode($dataTemplate);

    // Generate missing entries for Hisuian Pokemon
    $pokemonIds = sgg_get_sorted_pokemon_ids();
    foreach ($pokemonIds as $pkmId) {
        $pkmFile = 'pokemon/entries/' . $pkmId . '.json';
        if (!file_exists(sgg_get_data_path($pkmFile))) {
            $isHisuian = str_contains($pkmId, '-hisui') || str_contains($pkmId, 'white-stripe');
            $newPkm = $dataTemplateArr;
            $newPkm['id'] = $pkmId;
            $newPkm['name'] = ucfirst($pkmId);
            $newPkm['isRegional'] = $isHisuian;
            $newPkm['isForm'] = str_contains($pkmId, '-');
            $newPkm['region'] = 'hisui';
            $newPkm['generation'] = 8;
            sgg_data_save($pkmFile, $newPkm, false);
        }
    }

    $dataSet = sgg_get_merged_pkm_entries();
    foreach ($dataSet as $i => $pkm) {
        $pkmId = $pkm['id'];

        $outputFile = 'pokemon/entries/' . $pkmId . '.json';

        /* --- START $newPkm data transformation */

        $newPkm = array_merge($dataTemplateArr, $pkm);

        /* --- END $newPkm data transformation */
        // save pkm entry
        sgg_data_save($outputFile, $newPkm, false);
    }

    echo "[OK] Build finished!\n";
})();
